    ...
    if len(parent_node)==0:
        ...
        if file_name.split('.')[-1]=='csv' or file_name.split('.')[-1]=='json':
            ...
            for i in range(0,len(num_list)):
                cursor.execute("CREATE TABLE t_"+str(last_node)+"_"+str(num_list[i])+" (content varchar(600) not null);")
                value = '{"t_'+str(last_node)+'_'+str(num_list[i])+'":'+'"'+content_list[i]+'"}'  
                print(r.patch(firebase_link+'.json',data=value).json()) 
                link=firebase_link+'t_'+str(last_node)+'_'+str(num_list[i])+'.json'
                cursor.execute("INSERT INTO t_"+str(last_node)+'_'+str(num_list[i])+" Values('"+link+"');")
