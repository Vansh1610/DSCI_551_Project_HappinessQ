...
def analytics():
    ...
    if 'whr' in paths.keys():
        whr_path=paths['whr']['path']
    if 'GDP' in paths.keys():
        gdp_path=paths['GDP']['path']
    if 'unemp' in paths.keys():
        unemp_path=paths['unemp']['path']
    ...
    return render_template('analytics.html',ans=[ans],exp=exp,csv=csv,paths=paths)
